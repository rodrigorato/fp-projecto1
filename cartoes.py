...
def duplicaImparesESubtraiNove(cadeia):
    '''Recebe uma string e devolve a string com os numeros em indices impares multiplicados por dois e caso esses numeros sejam superiores a 9 e' lhes subtraido o valor 9. Esta string tera' de ser uma em que para qualquer que seja o numero inteiro n, eval(cadeia[n]) existe e e' um numero inteiro.'''
    duplicados = ""
    n = 0
...
def valida_iin(cadeia):
    '''Usa os tuplos definidos no inicio do ficheiro para validar o iin da cadeia que a funcao recebe.Recebe o numero de um cartao e devolve o nome da rede emissora em string.'''    
...
def numero_digitos(cad1, tup1):
    '''Recebe uma cadeia e um tuplo de inteiros e verifica se o comprimento da cadeia e' igual a algum dos inteiros no tuplo.'''
...
def categoria(cadeia):
    '''Recebe uma cadeia e verifica o se o que esta' no indice 0 dessa cadeia e' alguma categoria conhecida e devolve a cadeia correspondente 'a categoria. Em caso contrario devolve a cadeia de caracteres vazia.'''
    if cadeia[0] == "1":
...
def verifica_cc(numero_cartao):
    '''Recebe um numero de um cartao e verifica se e' valido, ou seja: e' verificado pelo algoritmo de Luhn, tem uma categoria valida associada ao mesmo e o seu iin pode ser validado. Devolve o tuplo correspondente as strings da categoria e do nome da rede emissora. Em caso de um cartao inva'lido devolve a cadeia de caracteres "cartao invalido".'''
...
def randomCartaoSemVerificacao(abrev):
    '''Recebe abreviatura da rede emissora, se for valida gera o numero aleatoriamente pela funcao encheRandomAte para a qual passa como argumentos o digito inicial (ou digitos iniciais, se for o caso usa escolheElementoTuplo para escolher um prefixo aleatoriamente) e o numero de digitos possiveis menos um, o digito de verificacao. Em caso de diferentes "comprimentos" possiveis escolhe do tuplo dos comprimentos um aleatoriamente, como para os prefixos. Devolve o inteiro do numero do cartao sem dig. de verificacao.'''
    num = 0
    if abrev == "AE":
...
def tuploInteirosTuploCadeias(tuplo_ints):
    '''Recebe um tuplo com membros do tipo inteiro e devolve um tuplo com as cadeias de caracteres correspondentes a esses inteiros.'''
    tuplo_cadeias = ()
...
def gera_num_cc(abrev):
    '''Gera um numero de cartao aleatorio dada a abreviatura da rede emissora como parametro, devolve um numero de um cartao valido (inteiro) que pertence 'a rede desejada.'''
    num = 0
...
def encheRandomAte(numero, comprimento):
    '''Devolve um inteiro "completado", aleatoriamente, com prefixo de valor igual 'a variavel numero, ate' o seu comprimento ser igual ao valor do inteiro comprimento.'''
...
def randomZeroANum(num):
    '''Escolhe um inteiro aleatoriamente desde 0, ate' o valor da variavel/parametro num. E' possivel obter tanto o numero 0 como o valor de num.'''
...
def escolheElementoTuplo(tuplo_de_cadeias):
    '''Escolhe aleatoriamente um elemento de um tuplo cujos elementos sao strings e devolve o inteiro correspondente a esse elemento.'''
    tuplo_de_inteiros = ()
...
def digito_verificacao(cc_cartao_sem_ultimo):
    '''Calcula o digito de verificacao de uma cadeia de caracteres correspondente a um numero de um cartao sem este ultimo digito. Devolve apenas o ultimo digito.'''
